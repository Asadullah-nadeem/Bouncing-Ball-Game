...
public class LevelGenerator {
     ...
     */
    public static ArrayList<SpecialBrick> generateLevel(int level, Random random) {
        ...
        if (level <= 10) { // Easy levels with low density
            generateRandomLayout(bricks, startX, 0.4 + (level * 0.02), level, random);
        } else if (level <= 20) { // Medium levels with higher density
            generateRandomLayout(bricks, startX, 0.6 + ((level - 10) * 0.01), level, random);
        } else { // Hard levels with patterns
            int patternType = (level - 21) % 5; // Cycle through 5 patterns for variety
            switch (patternType) {
                case 0:
                    generateCheckerboardLayout(bricks, startX, level, random);
                    break;
                case 1:
                    generateSolidWithGapsLayout(bricks, startX, level, random);
                    break;
                case 2:
                    generatePyramidLayout(bricks, startX, level, random);
                    break;
                case 3:
                    generateVerticalLinesLayout(bricks, startX, level, random);
                    break;
                default:
                    generateRandomLayout(bricks, startX, 0.85, level, random);
                    break;
            }
        }
        return bricks;
    ...
    private static void addBrick(ArrayList<SpecialBrick> bricks, int x, int y, int level, Random random) {
        PowerUpType powerUpType = determinePowerUpType(level, random);
    ...
    private static PowerUpType determinePowerUpType(int level, Random random) {
        // Small chance for any brick to become a power-up brick
    ...
    private static void generateRandomLayout(ArrayList<SpecialBrick> bricks, int startX, double density, int level, Random random) {
        for (int row = 0; row < GameConstants.BRICK_ROWS; row++) {
            for (int col = 0; col < GameConstants.BRICK_COLS; col++) {
                if (random.nextDouble() < density) {
                    addBrick(bricks, startX + col * GameConstants.BRICK_WIDTH, row * GameConstants.BRICK_HEIGHT + 50, level, random);
                }
            }
        }
    }

    private static void generateCheckerboardLayout(ArrayList<SpecialBrick> bricks, int startX, int level, Random random) {
        for (int row = 0; row < GameConstants.BRICK_ROWS; row++) {
            for (int col = 0; col < GameConstants.BRICK_COLS; col++) {
                if ((row + col) % 2 == 0) {
                    addBrick(bricks, startX + col * GameConstants.BRICK_WIDTH, row * GameConstants.BRICK_HEIGHT + 50, level, random);
    ...
    private static void generateSolidWithGapsLayout(ArrayList<SpecialBrick> bricks, int startX, int level, Random random) {
        for (int row = 0; row < GameConstants.BRICK_ROWS; row++) {
    ...
    private static void generatePyramidLayout(ArrayList<SpecialBrick> bricks, int startX, int level, Random random) {
    ...
    private static void generateVerticalLinesLayout(ArrayList<SpecialBrick> bricks, int startX, int level, Random random) {
        for (int row = 0; row < GameConstants.BRICK_ROWS; row++) {
            for (int col = 0; col < GameConstants.BRICK_COLS; col++) {
                if (col % 3 == 0) {
                    addBrick(bricks, startX + col * GameConstants.BRICK_WIDTH, row * GameConstants.BRICK_HEIGHT + 50, level, random);
